...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const runJustifiedGallery = function (ele) {
    // 阿里云 OSS 图片处理：页面缩略图用压缩小图避免解码相机原图导致卡顿，fancybox 点开仍加载原图
    const canOssProcess = url => /\.aliyuncs\.com\//.test(url) && !url.includes("?");
    const ossThumb = url => (canOssProcess(url) ? `${url}?x-oss-process=image/resize,w_800/quality,q_80/format,webp` : url);
    const htmlStr = arr => {
      let str = "";
      ...
      arr.forEach(i => {
        ...
        <div class="fj-gallery-item">
          ...
          <img src="${ossThumb(i.url)}" data-fancybox-href="${i.url}" decoding="async" ${alt + title}>
        </div>
      `;
        str += galleryItem;
      });
  ...
  // 常驻的无限动画(首页技能图标墙、文章顶部波浪)在离屏或页面隐藏时暂停,减少 GPU 占用
  let infiniteAnimObserver = null;
  const pausableAnimEls = [];

  const updateAnimPauseState = el => {
    el.classList.toggle("anim-paused", document.hidden || el.dataset.animOffscreen === "true");
  };

  const pauseAnimWhenInvisible = function () {
    infiniteAnimObserver && infiniteAnimObserver.disconnect();
    pausableAnimEls.length = 0;
    const els = document.querySelectorAll("#skills-tags-group-all .tags-group-wrapper, .main-hero-waves-area");
    if (!els.length) return;
    infiniteAnimObserver = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        entry.target.dataset.animOffscreen = entry.isIntersecting ? "false" : "true";
        updateAnimPauseState(entry.target);
      });
    });
    els.forEach(el => {
      pausableAnimEls.push(el);
      infiniteAnimObserver.observe(el);
    });
  };

  const unRefreshFn = function () {
    document.addEventListener("visibilitychange", () => {
      pausableAnimEls.forEach(updateAnimPauseState);
    });
