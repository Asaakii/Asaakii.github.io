const anzhiyu = {
    ...
    if (service === "fancybox") {
      Array.from(ele).forEach(i => {
        if (i.parentNode.tagName !== "A") {
          const thumbSrc = i.dataset.lazySrc || i.src;
          // data-fancybox-href 存放大图地址（如 OSS 处理后的中等尺寸图），点开时加载大图而非页面缩略图
          const dataSrc = i.dataset.fancyboxHref || thumbSrc;
  ...
  // 同步音乐馆 Apple Music 风格封面、标题和播放状态
  syncMusicPageUI: function () {
    const anMusicPage = document.getElementById("anMusic-page");
    if (!anMusicPage) return;

    const musiccover = anMusicPage.querySelector(".aplayer-pic");
    const coverImage = musiccover?.style?.backgroundImage;
    if (coverImage) anMusicPage.style.setProperty("--am-cover-image", coverImage);

    const title = anMusicPage.querySelector(".aplayer-title")?.textContent?.trim() || "等待播放";
    const artist =
      anMusicPage.querySelector(".aplayer-author")?.textContent?.replace(/^\s*-\s*/, "").trim() || "从歌单里选一首歌";
    const titleEl = anMusicPage.querySelector(".am-title");
    const artistEl = anMusicPage.querySelector(".am-artist");
    if (titleEl) titleEl.textContent = title;
    if (artistEl) artistEl.textContent = artist;

    const metingAplayer = anMusicPage.querySelector("meting-js")?.aplayer;
    anMusicPage.classList.toggle("is-playing", !!metingAplayer && !metingAplayer.audio.paused);
    anzhiyu.ensureMusicPageControllerMounted();
    anzhiyu.styleMusicPageControllerIcons();
  },
  mountMusicPageController: function () {
    const anMusicPage = document.getElementById("anMusic-page");
    if (!anMusicPage) return false;

    const controlDock = anMusicPage.querySelector(".am-control-dock");
    const controller = anMusicPage.querySelector(".aplayer-controller");
    if (!controlDock || !controller) return false;

    controller.classList.add("am-controller-mounted");
    if (!controlDock.contains(controller)) controlDock.appendChild(controller);
    anzhiyu.styleMusicPageControllerIcons();
    return true;
  },
  styleMusicPageControllerIcons: function () {
    const anMusicPage = document.getElementById("anMusic-page");
    if (!anMusicPage) return;

    const metingAplayer = anMusicPage.querySelector("meting-js")?.aplayer;
    const isPaused = !metingAplayer || metingAplayer.audio.paused;
    const orderMode = metingAplayer?.options?.order || "list";
    const loopMode = metingAplayer?.options?.loop || "all";
    const iconMap = [
      [".aplayer-icon-order", "anzhiyu-icon-shuffle"],
      [".aplayer-icon-back", "anzhiyu-icon-backward"],
      [".aplayer-icon-play", isPaused ? "anzhiyu-icon-play" : "anzhiyu-icon-pause"],
      [".aplayer-icon-forward", "anzhiyu-icon-forward"],
      [".aplayer-icon-loop", "anzhiyu-icon-repeat"]
    ];

    iconMap.forEach(([selector, iconClass]) => {
      const button = anMusicPage.querySelector(`.am-control-dock ${selector}`);
      if (!button) return;
      const icon = button.querySelector("i");
      if (icon) {
        icon.className = `anzhiyufont ${iconClass}`;
      } else {
        button.insertAdjacentHTML("beforeend", `<i class="anzhiyufont ${iconClass}"></i>`);
      }

      if (selector === ".aplayer-icon-order") {
        button.dataset.amMode = orderMode;
        button.setAttribute("aria-label", orderMode === "random" ? "随机播放已开启" : "开启随机播放");
        button.setAttribute("title", orderMode === "random" ? "随机播放已开启" : "开启随机播放");
      }

      if (selector === ".aplayer-icon-loop") {
        button.dataset.amMode = loopMode;
        let singleLoopBadge = button.querySelector(".am-loop-one-badge");
        if (loopMode === "one") {
          if (!singleLoopBadge) {
            button.insertAdjacentHTML("beforeend", '<span class="am-loop-one-badge" aria-hidden="true">1</span>');
          }
        } else if (singleLoopBadge) {
          singleLoopBadge.remove();
        }
        button.setAttribute(
          "aria-label",
          loopMode === "none" ? "开启循环播放" : loopMode === "one" ? "单曲循环" : "列表循环"
        );
        button.setAttribute("title", button.getAttribute("aria-label"));
      }

      if (!button.dataset.amRestyleBound && (selector === ".aplayer-icon-order" || selector === ".aplayer-icon-loop")) {
        button.dataset.amRestyleBound = "true";
        button.addEventListener("click", () => {
          requestAnimationFrame(() => anzhiyu.styleMusicPageControllerIcons());
        });
      }
    });
  },
  ensureMusicPageControllerMounted: function () {
    let retryCount = 0;
    const mount = () => {
      const mounted = anzhiyu.mountMusicPageController();
  ...
  setMusicPageView: function (view = "lyrics") {
    const anMusicPage = document.getElementById("anMusic-page");
    if (!anMusicPage) return;

    const isQueue = view === "queue";
    anMusicPage.classList.toggle("am-queue-open", isQueue);
    anMusicPage.querySelectorAll(".am-view-btn").forEach(button => {
      const isActive = button.dataset.amView === view;
  ...
  changeMusicBg: function (isChangeBg = true) {
    ...
    if (isChangeBg) {
      ...
      if (musiccover) {
        const coverImage = musiccover.style.backgroundImage;
        if (anMusicBg) anMusicBg.style.backgroundImage = coverImage;
        document.getElementById("anMusic-page")?.style.setProperty("--am-cover-image", coverImage);
      }
      anzhiyu.syncMusicPageUI();
    } else {
      // 第一次进入，绑定事件，改背景
  ...
  addEventListenerMusic: function () {
    ...
    const musicViewButtons = anMusicPage.querySelectorAll(".am-view-btn");
    const metingAplayer = anMusicPage.querySelector("meting-js").aplayer;
    //初始化音量
    metingAplayer.volume(0.8, true);
    ["loadeddata", "listswitch", "play", "pause"].forEach(eventName => {
      metingAplayer.on(eventName, function () {
        anzhiyu.changeMusicBg();
      });
    });
    anzhiyu.syncMusicPageUI();
    ...
    if (aplayerIconMenu) {
      aplayerIconMenu.addEventListener("click", function () {
        ...
        const aplayerList = anMusicPage.querySelector(".aplayer.aplayer-withlist .aplayer-list");
        if (aplayerList) aplayerList.style.opacity = "1";
      });
    }
    ...
    const menuMask = document.getElementById("menu-mask");
    if (menuMask) menuMask.addEventListener("click", anMusicPageMenuAask);

    musicViewButtons.forEach(button => {
      if (button.dataset.boundMusicView === "true") return;
      button.dataset.boundMusicView = "true";
      button.addEventListener("click", () => {
        anzhiyu.setMusicPageView(button.dataset.amView || "lyrics");
      });
    });
    anzhiyu.setMusicPageView(anMusicPage.classList.contains("am-queue-open") ? "queue" : "lyrics");
